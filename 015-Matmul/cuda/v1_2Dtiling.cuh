...
// 2D thread tiling：每线程算 TM×TN 块，内层外积，算访比 → TM*TN/(TM+TN)
template <int BM = 128, int BN = 128, int BK = 8, int TM = 8, int TN = 8>
__global__ void sgemm_v1_2(const float* __restrict__ A,
                           const float* __restrict__ B,
                           float* __restrict__ C,
                           int M, int N, int K) {
  constexpr int NT = (BM / TM) * (BN / TN);            // 线程数 = 256
  __shared__ float sA[BM][BK], sB[BK][BN];

  const int bx = blockIdx.x, by = blockIdx.y;
  const int tx = threadIdx.x, ty = threadIdx.y;        // blockDim: (BN/TN, BM/TM)
  const int tid = ty * (BN / TN) + tx;

  float accum[TM][TN] = {{0.f}};                       // TM*TN 累加器住寄存器

  for (int bk = 0; bk < (K + BK - 1) / BK; ++bk) {
    const int gk = bk * BK;

    // 协作搬运 sA(BM*BK)：每线程搬 BM*BK/NT 个，按 NT 步长覆盖
    #pragma unroll
