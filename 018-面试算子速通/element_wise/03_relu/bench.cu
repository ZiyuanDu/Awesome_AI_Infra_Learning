...
static void reluCpu(const float* in, float* out, int N) {
    for (int i = 0; i < N; ++i)
...
static void test(int N, const char* name, double bytes = 0) {
    ...
    for (int i = 0; i < N; ++i)
        in[i] = (float)((i % 13) - 6);
    bench(name, {&in}, N,
          [=](const float* const* a, float* out) { reluCpu(a[0], out, N); },
...
int main() {
    ...
    bench("ex", {&ex}, ex.size(),
          [&](const float* const* a, float* out) { reluCpu(a[0], out, (int)ex.size()); },
